...
public interface ICustomerDal extends CrudRepository<Customer, Long> {
  ...
  @Query("SELECT new com.sari.server.beans.CustomerDto(cus.id, cus.name, cus.address, cus.phoneNumber) FROM Customer cus WHERE cus.id = :id")
  CustomerDto findById(@Param("id") long id);

  @Query("SELECT new com.sari.server.beans.CustomerDto(cus.id, cus.name, cus.address, cus.phoneNumber) FROM Customer cus")
