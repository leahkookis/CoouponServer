...
public interface IPurchaseDal extends CrudRepository<Purchase, Long> {
   ...
   @Query("SELECT new com.sari.server.beans.PurchaseDto(pur.id, pur.customer.name, pur.coupon.price, pur.coupon.name, pur.timeStamp, pur.coupon.category.name, pur.coupon.company.name,pur.isBuy)" +
