...
public interface ICouponDal extends CrudRepository<Coupon, Long> {
    ...
    @Query("SELECT new com.lea.server.beans.CouponDto(coup.id, coup.name, coup.price, coup.description, coup.startDate, coup.endDate, coup.category.name,coup.category.id, coup.company.name,coup.company.id, coup.amount, coup.url) " +
